...
func _process(_delta):
	# Kamera sallanmasÄ± iÃ§in _process iÃ§inde ufak sarsÄ±ntÄ±lar da kullanabiliriz (kitap mekaniÄŸindeki gibi)
...
func yuzugu_al_ve_isinlan():
	...
	# TRAVMA EFEKTÄ° (Ekran kÄ±zarma/kararma efekti)
	var canvas = CanvasLayer.new()
	canvas.layer = 98
	get_tree().current_scene.add_child(canvas)
	var cr = ColorRect.new()
	cr.set_anchors_preset(Control.PRESET_FULL_RECT)
	cr.color = Color(0.4, 0.0, 0.0, 0.0) # Koyu kÄ±rmÄ±zÄ±
	...
	# Ekrani tamamen siyaha gecirip sahne degistirme:
	var siyah_ekran = create_tween()
	siyah_ekran.tween_property(cr, "color", Color(0,0,0,1), 0.5)
	await siyah_ekran.finished
	
	# KALEYE IÅINLANMA
	# Oyuncu donmasÄ±nÄ± Ã§Ã¶z (DiÄŸer sahnede de kilitli kalmamasÄ± iÃ§in)
	if is_instance_valid(donmus_oyuncu):
