extends Node
# Sihirli kitap etkilesim mekanizmasi.
# Kitap okunca: oyuncu kitlenir, kriz geciriyor, kapi acilir, oyuncu serbest kalir.
# Sahne gecisi YOK — oyuncu kapidan yuruyor cikiyor.
...
func kitap_etkilesimi(oyuncu):
	...
	# Kitabi SihirliKitap grubundan cikar - artik etkilesilemez
	...
	# 1.5 ETKİLEŞİM YAZISINI GİZLE ("Kitabı İncele {E}")
	if is_instance_valid(oyuncu):
	...
	# 3. YENİ TRAVMA EFEKTİ (Kalp atışı kırmızılık)
	var canvas = CanvasLayer.new()
	canvas.layer = 98
	get_tree().current_scene.add_child(canvas)
	var cr = ColorRect.new()
	cr.set_anchors_preset(Control.PRESET_FULL_RECT)
	cr.color = Color(0.4, 0.0, 0.0, 0.0) # Koyu kırmızı, başta saydam
	cr.mouse_filter = Control.MOUSE_FILTER_IGNORE # Ekranın tıklanmasını engellemesini iptal et
	canvas.add_child(cr)
	...
	# Renk efektini durdur ve temizle
	...
	# 7. OYUNCUYA KONTROLU GERİ VER VE KALICI TRAVMAYI BAŞLAT
	...
	# ÖNEMLİ: Kitap okundu bayrağını set et
	# anahtar_sonu_tetigi.gd ve yatak_tetigi.gd bunu kontrol eder
	...
	# NOT: Sahne gecisi YOK!
	# Oyuncu kapidan yuruyerek cikar. Oda gecisi kutuphane disinda
	# bir Area3D trigger tarafindan yonetilecek (anahtar_sonu_tetigi veya yeni bir trigger).
